...
const { runUnsafeScript: installScript } = require('./isolate/script-runner.js')
...
async function run(script, ...args) {
    return await runWithIsolatedContext({}, async ({ }, { run, collectAndReleaseRefs, installUntrustedScript }) => {
        let logs, error, result, durationMillis

        try {
            await installUntrustedScript(script)
            const ref = collectAndReleaseRefs(createTransferableReference({ args }))
            const fnResult = collectAndReleaseRefs(await run(ref))
