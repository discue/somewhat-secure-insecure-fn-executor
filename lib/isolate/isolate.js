...
const { runUnsafeScript, runSafeScript } = require('./script-runner.js')
    ...
    try {
        return await callback.call(null, { isolate, context }, {
            ...
            installTrustedScript: async (script) => {
                return runSafeScript({ isolate, context, script })
            },
            /**
             * 
             * @param {string} script the script to run
             */
            installUntrustedScript: async (script) => {
                return runUnsafeScript({ isolate, context, script })
            }
        })
    } finally {
        collectedRefs
